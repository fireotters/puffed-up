...
namespace ExtensionsFunctions
{
    public static class VectorExtensions
    {
        ...
        public static Vector3[] GetIntermediatePositions(Vector3 start, Vector3 end, int intermediatePostitions)
        {
            ...
            var result = new Vector3[intermediatePostitions];
            var step = 1.0f / (intermediatePostitions + 1);

            for (var i = 0; i < intermediatePostitions; i++)
            {
        ...
        {
            var step = 1.0f / (numOfPositions + 1);
        ...
        /// <param name="intermediatePostitions"></param>
        /// <returns></returns>
        public static Vector3[] GetIntermediatePositionsInclusive(Vector3 start, Vector3 end,
            int intermediatePostitions)
        {
            var result = new Vector3[intermediatePostitions + 2];
            result[0] = start;
            result[intermediatePostitions + 1] = end;
            var step = 1.0f / (intermediatePostitions + 1);

            for (var i = 1; i < intermediatePostitions + 1; i++)
            {
